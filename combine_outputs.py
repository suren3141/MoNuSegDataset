...
def combine_output_single(img_file, label_file, sample_file):
    ...
    out_path = os.path.join(os.path.pardir(file_path), "combined")

    if not os.path.exists(out_path): os.mkidr(out_path)
...
img_files = sorted(glob(path, "images", "*.png"))
label_files = sorted(glob(path, "labels", "*.png"))
sample_files = sorted(glob(path, "samples", "*.png"))
